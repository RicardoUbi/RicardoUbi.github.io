  ...
  return (
    <Stack gap="xl">
      ...
      <Text size="xl" c="winterBlack.8" fw={300} maw={800} style={{ lineHeight: 1.6, letterSpacing: '-0.01em' }}>
        Adoro criar soluções eficientes para problemas complexos.

        Minha paixão está em desenvolver ferramentas que resolvam problemas reais e impactem positivamente a vida de quem as utiliza.

        Acredito que um software cumpre seu papel quando é intuitivo e útil.
      </Text>
            ...
            {EXPERIENCES.map((exp, index) => (
              <Box key={index}>
                <Group justify="space-between" align="flex-start" mb={4}>
                  ...
                  <Text ff="monospace" fz="0.7rem" c="winterBlack.3">{exp.period}</Text>
                </Group>
                <Text fz="xs" fw={700} tt="uppercase" lts="0.1em" c="winterBlack.4" mb="md">
                  {exp.company}
                </Text>
                <Text size="sm" c="winterBlack.6" style={{ lineHeight: 1.6 }} fw={300}>
                  {exp.description}
                </Text>
              </Box>
            ))}
          ...
          <Stack gap="xl">
            {EDUCATION.map((edu, index) => (
              <Box key={index}>
                ...
                <Text ff="monospace" size="xs" c="winterBlack.3" mt={4}>{edu.period}</Text>
              </Box>
            ))}

            <Divider color="winterBlack.1" label="Certificações" labelPosition="left" styles={{ label: { fontSize: '0.6rem', textTransform: 'uppercase', letterSpacing: '0.2em' } }} />
            ...
            <Stack gap="sm">
              {CERTIFICATIONS.map((cert, index) => (
                <Group key={index} gap="xs" wrap="nowrap" style={{ opacity: 0.7, transition: '0.2s', cursor: 'default' }}>
                  ...
                  <Text size="xs" c="winterBlack.7" fw={400}>
                    {cert}
                  </Text>
                </Group>
              ))}
